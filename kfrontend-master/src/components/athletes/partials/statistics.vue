<template>
	...
	<div v-else>
		...
		<div
			...
			<FormTemplate
				v-if="store.getters.getUserRole === 'ADMIN' && props.showPenalizationForm"
				class="mt-4"
			...
			<table v-else-if="penalizations.results.length" class="table-auto w-full mt-4">
				<thead>
					<tr>
						...
						<th
							v-if="
								store.getters.getUserRole === 'ADMIN' && props.showPenalizationForm
							">
							{{ t("delete") }}
						</th>
					</tr>
			...
			<p v-else class="my-2 font-medium">{{ t("noPenalizations") }}</p>
		</div>
...
const loadingPenalizationList = ref(props.immediate);
const penalizations = ref({ results: [] });
...
const getResults = () => {
	...
	if (calculateWithPenalizations.value) {
...
const getPenalizations = async () => {
	...
	try {
		const { data } = await unauthApi.get(props.penalizationUrl);
